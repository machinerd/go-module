...
## 📁 패키지 구성 및 역할
- **`/audit`**: 데이터 로그 저장과 관련된 파일들이 위치합니다.
...
- **`/log`** : slog 기반 구조화 로깅(레벨 제어, JSON 출력)을 추상화한 공통 로거가 위치합니다.
    - **`/gqlgen`**: gqlgen 서브그래프 공통 로깅(오퍼레이션 시작/종료, 패닉 리커버리, dataloader 배치 트레이싱)을 담당합니다.
